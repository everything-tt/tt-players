...
/**
 * Derives one observation's evidence from the same Glicko-2 terms used by the
 * period update. Attributed deltas for all observations in a rating period sum
 * to the period's combined rating delta, so no artificial within-day ordering
 * is required.
 */
