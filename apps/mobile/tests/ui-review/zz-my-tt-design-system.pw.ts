...
const player = { id: 'my-tt-design-system-player', name: 'Wudong Liu' };
const userId = '11111111-1111-4111-8111-111111111111';
const email = 'my-tt-design-system@example.test';
...
test('uses one hero and flat design-system sections across My TT', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.tt-metric')).toHaveCount(3);
  await expect(page.locator('.tt-section--raised')).toHaveCount(0);
  await expect(page.locator('.tt-my-tt-flat-section')).toHaveCount(3);
  await expect(page.getByText('Butterfly Viscaria')).toBeVisible();
  await expect(page.getByText('Account-owned profile')).toBeVisible();

  const filledGeometry = await page.evaluate(() => {
    const hero = document.querySelector<HTMLElement>('.tt-entity-hero')!.getBoundingClientRect();
    return {
      viewport: window.innerWidth,
      scrollWidth: document.documentElement.scrollWidth,
      heroHeight: hero.height,
      raisedSections: document.querySelectorAll('.tt-section--raised').length,
      flatSections: document.querySelectorAll('.tt-section--flat').length,
    };
  });
  expect(filledGeometry.scrollWidth).toBeLessThanOrEqual(filledGeometry.viewport + 1);
  expect(filledGeometry.heroHeight).toBeLessThan(300);
  expect(filledGeometry.raisedSections).toBe(0);
  ...
  await expect(page.getByText('Add a short introduction')).toBeVisible();
  await expect(page.locator('.tt-section--raised')).toHaveCount(0);
  ...
  const emptyGeometry = await page.evaluate(() => ({
    ...
    scrollWidth: document.documentElement.scrollWidth,
    raisedSections: document.querySelectorAll('.tt-section--raised').length,
    clickableRows: document.querySelectorAll('.tt-list-item__clickable').length,
  }));
  expect(emptyGeometry.scrollWidth).toBeLessThanOrEqual(emptyGeometry.viewport + 1);
  expect(emptyGeometry.raisedSections).toBe(0);
  expect(emptyGeometry.clickableRows).toBeGreaterThanOrEqual(3);
  await capture(page, testInfo, 'my-tt-flat-empty-profile', emptyGeometry);
  ...
  await expect(page.locator('.tt-my-tt-edit-section')).toHaveCount(6);
  await expect(page.locator('.tt-section--raised')).toHaveCount(0);
  await expect(page.getByLabel('Dominant shot')).toHaveValue('Forehand loop');
  await expect(page.getByRole('button', { name: 'Save changes' })).toBeDisabled();
  await expect(page.locator('.tt-toggle-button')).toHaveCount(17);

  const dockGeometry = await page.evaluate(() => {
    const dock = document.querySelector<HTMLElement>('.tt-my-tt-save-dock')!;
  ...
  expect(dockGeometry.scrollWidth).toBeLessThanOrEqual(dockGeometry.viewport + 1);
  expect(dockGeometry.left).toBe(0);
  expect(dockGeometry.rightGap).toBe(0);
  expect(dockGeometry.borderRadius).toBe('0px');
  expect(dockGeometry.boxShadow).toBe('none');

  await page.getByRole('button', { name: 'Attacking' }).click();
  await expect(page.getByText('Unsaved changes')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Save changes' })).toBeEnabled();
  await capture(page, testInfo, 'my-tt-flat-editor', dockGeometry);
