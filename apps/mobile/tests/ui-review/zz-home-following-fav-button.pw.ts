...
async function prepareAppState(page: Page) {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify([
        { id: 'p1', name: 'James Donald', played: 1025, wins: 620 },
        { id: 'p2', name: 'Kuzey Musabak', played: 359, wins: 209 },
      ]),
    );
...
test('reviews direct favourite heart button on home page following list instead of overflow menu', async ({
  page,
}, testInfo) => {
  ...
  // Verify Following section header renders
  await expect(page.getByRole('heading', { name: 'Following' })).toBeVisible();

  // Verify James Donald row has a direct heart favourite button instead of 3-dots overflow menu
  const jamesRow = page.locator('.tt-list-item', { hasText: 'James Donald' });
  await expect(jamesRow).toBeVisible();

  const favButton = jamesRow.getByRole('button', { name: /Remove from favourites/i });
  await expect(favButton).toBeVisible();
  await expect(favButton.locator('i.fa.fa-heart')).toBeVisible();
  await expect(jamesRow.locator('button[aria-label*="Following actions"]')).toBeHidden();

  await capture(page, testInfo, 'home-following-fav-button');

  // Toggle favourite to unfollow James Donald
  await favButton.click();
  await expect(jamesRow).toBeHidden();

  await capture(page, testInfo, 'home-following-after-unfollow');
