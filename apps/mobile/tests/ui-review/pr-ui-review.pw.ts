/**
 * PR-SPECIFIC UI REVIEW — AGENT INSTRUCTIONS
 *
 * This is the only Playwright test executed by the UI PR screenshot pipeline.
 * Whenever an agent creates or substantially updates a UI PR:
 *
 * 1. Replace the scenarios in this file with focused coverage for that PR.
 * 2. Remove the previous PR's scenarios; do not keep appending unrelated tests.
 * 3. Exercise the changed user flows against the deployed preview.
 * 4. Assert the important behaviour and layout before taking screenshots.
 * 5. Capture only screenshots that help review the current PR.
 *
 * The broad route crawler in ui-review.pw.ts is intentionally excluded from
 * the PR pipeline. It may be run separately for a full-application audit.
 */
