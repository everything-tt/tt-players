...
async function expectTimelineLayout(page: Page, title: string) {
  await expect(page.getByRole('heading', { name: title })).toBeVisible();
  await expect(page.locator('.tt-tournament-timeline-group').first()).toBeVisible();
  await expect(page.locator('.tt-tournament-timeline-item').first()).toBeVisible();
  await expect(page.locator('.tt-tournament-date-tile').first()).toBeVisible();
  await expect(page.locator('.tt-tournament-timeline-item__status-row').first()).toBeVisible();

  const geometry = await page.locator('.tt-tournament-results-section').evaluate((section) => {
    const cards = Array.from(section.querySelectorAll<HTMLElement>('.tt-tournament-timeline-item'));
    const dateTiles = Array.from(section.querySelectorAll<HTMLElement>('.tt-tournament-date-tile'));
    return {
      cardCount: cards.length,
      matchingDateTileCount: dateTiles.length === cards.length,
  ...
  expect(geometry.matchingDateTileCount).toBe(true);
  expect(geometry.cardsContained).toBe(true);
  expect(geometry.pageContained).toBe(true);
}
...
test('reviews upcoming and completed tournament timeline layouts', async ({ page }, testInfo) => {
  ...
  await expectTimelineLayout(page, 'Upcoming tournaments');
  await expect(page.getByRole('textbox', { name: 'Search upcoming tournaments' })).toBeVisible();
  await expect(page.getByRole('button', { name: /show saved tournaments only/i })).toBeVisible();
  await capture(page, testInfo, 'tournaments-upcoming-timeline');
  ...
  await expectTimelineLayout(page, 'Completed tournaments');
  await expect(page.getByRole('textbox', { name: 'Search completed tournaments' })).toBeVisible();
  await capture(page, testInfo, 'tournaments-completed-timeline');

  await page.setViewportSize({ width: 320, height: 844 });
  await page.getByRole('radio', { name: 'Upcoming' }).click();
  await expectTimelineLayout(page, 'Upcoming tournaments');
  await capture(page, testInfo, 'tournaments-upcoming-timeline-320');
