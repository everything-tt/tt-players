...
function healthMeta(health: SourceHealth) {
  if (health === 'healthy') return { label: 'Healthy', icon: 'fa fa-check', tone: 'success' as const };
  if (health === 'degraded') return { label: 'Needs attention', icon: 'fa fa-exclamation', tone: 'danger' as const };
          ...
          <>
            <PageSection surface="flat" density="compact" title="Quality Summary" note={`${data.summary.providers} providers`}>
              <DesignList density="compact" divider="hairline" paginate={false}>
                ...
                <ListItem leading={<IconCircle iconClassName="fa fa-link" tone="accent" />} title={`${formatNumber(data.summary.pending_identity_suggestions)} identity suggestions awaiting review`} subtitle={`${formatNumber(data.summary.missing_player_rubbers)} results are missing a singles player`} hideChevron />
                <ListItem
                  leading={<IconCircle iconClassName={data.summary.unhealthy_resources > 0 ? 'fa fa-exclamation' : 'fa fa-check'} tone={data.summary.unhealthy_resources > 0 ? 'danger' : 'success'} />}
            ...
            {data.sources.map((source) => {
              ...
              return (
                <PageSection key={source.platform_id} surface="flat" density="compact" title={source.name} note={meta.label}>
                  <DesignList density="compact" divider="hairline" paginate={false}>
                    <ListItem leading={<IconCircle iconClassName={meta.icon} tone={meta.tone} />} title={meta.label} subtitle={`Latest activity: ${activityLabel(source.latest_activity_at)}`} hideChevron />
                    <ListItem leading={<IconCircle iconClassName="fa fa-layer-group" tone="accent" />} title={`${formatNumber(source.leagues)} leagues · ${formatNumber(source.competitions)} competitions`} subtitle={`${formatNumber(source.fixtures)} fixtures · ${formatNumber(source.rubbers)} results`} hideChevron />
                    <ListItem leading={<IconCircle iconClassName="fa fa-chart-bar" tone="success" />} title={`${source.dated_rubbers_pct}% dated · ${source.full_score_rubbers_pct}% full scores`} subtitle={`${formatNumber(source.canonical_players)} canonical players from ${formatNumber(source.external_players)} source profiles`} hideChevron />
                    <ListItem leading={<IconCircle iconClassName="fa fa-sync" tone="accent" />} title={`${formatNumber(source.total_scrapes)} stored payloads · ${formatNumber(source.failed_scrapes)} failed transforms`} subtitle={`${formatNumber(source.source_instances)} registered instances · ${formatNumber(source.source_resources)} resources`} hideChevron />
                    {source.last_error ? <ListItem leading={<IconCircle iconClassName="fa fa-bug" tone="danger" />} title="Latest resource error" subtitle={source.last_error} hideChevron /> : null}
                  </DesignList>
                  <AppButton full tone="outline" onClick={() => window.open(source.base_url, '_blank', 'noopener,noreferrer')}>Open Source Website</AppButton>
                </PageSection>
              );
            })}
