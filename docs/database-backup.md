...
The verifier downloads the dump/checksum/metadata, checks SHA-256, validates the archive catalog, restores into an automatically generated `tt_players_restore_*` database, confirms application tables exist, and drops the temporary database on exit. It never accepts a caller-provided restore database name, so it cannot overwrite `tt_players`.
