...
import { bigQuerySchema, bqIdentifier, commitRunSql, createDestinationSql, exportSql, failureRunSql, highWatermarkSql, mergeSql, pipelineBootstrapSql, readWatermarkSql, replaceEmptySql, replaceSql, resetWatermarkSql, stageValidationSql } from './bigquery-sql.mjs';
...
function validateStage(table, stagingTable, expectedRows) {
  ...
  if (actualRows !== expectedRows) throw new Error(`${table.destinationTable} row-count mismatch: source=${expectedRows}, staged=${actualRows}`);
  if (nullKeys !== 0 || duplicateKeys !== 0) throw new Error(`${table.destinationTable} key validation failed: null=${nullKeys}, duplicates=${duplicateKeys}`);
}
...
async function syncTable(table, tempRoot) {
  ...
  const runId = `${startedAt.replaceAll(/[-:.TZ]/g, '')}-${process.pid}-${table.destinationTable}`;
  const stageTable = `_stage_${table.destinationTable}_${runId.replaceAll(/[^a-zA-Z0-9_]/g, '_')}`.slice(0, 100);
  ...
  const { rows } = await psqlExport(exportSql(table, { lowerWatermark: prior?.timestamp ?? null, highWatermark: effectiveMode === 'incremental-merge' ? high : null }), dataPath);
  ...
  try {
    ...
    loadStage({ stagingTable: stageTable, gcsUri, schemaPath }); validateStage(table, stageTable, rows);
