...
TTP_BQ_MAX_BYTES_BILLED=30000000000
...
The 30 GB default leaves room for the current `raw_scrape_logs` replacement,
which scans just over 10 GB after JSON expansion, and its incremental MERGE,
which scans the staged overlap plus the destination table at just over 20 GB.
Increase `TTP_BQ_MAX_BYTES_BILLED` only after reviewing the job estimate and
expected monthly query volume; a query that exceeds the cap fails before
publication.
