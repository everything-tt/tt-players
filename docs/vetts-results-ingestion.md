...
The adapter key is `tournamentsoftware-vetts` and its current parser version is `1.4.0`. Its declared resource types are `directory`, `event`, and `event-results`. Recurring discovery, one-off backfills, tournament overviews, and result pages all execute through the adapter's `extract`/`transform` contract.
