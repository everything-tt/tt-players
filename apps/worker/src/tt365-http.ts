...
export async function fetchWithTT365Policy(
    ...
    const timeoutMs = envNumber('TT365_FETCH_TIMEOUT_MS', DEFAULT_TIMEOUT_MS);
    const maxAttempts = Math.max(
        1,
        Math.floor(envNumber('TT365_FETCH_MAX_ATTEMPTS', DEFAULT_MAX_ATTEMPTS)),
    );
    ...
    throw new Error(`TT365 request failed after ${maxAttempts} attempts: ${url}`);
}
